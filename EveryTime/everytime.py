...
class Crawler:
    ...
    def com_crawl(self):
        for page in range(1, 11): #10개의 페이지씩
            time.sleep(0.5)

            #첫 페이지 설정(10개씩 나눠서 진행할때 11, 21, ..등의 페이지부터 시작할 수 있도록)
            ...
            for count in range(1, 21): #한 페이지당 20개의 글
                try: #접근이 안되는 게시글 오류로 인한 실행종료 방지
                    ...
                    like = driver.find_element_by_class_name('vote').text
        ...
        return self._title, self._content, self._like, self._comment
...
# result_seoul 파일 생성
# with open('result_seoul_5.csv', 'w', encoding='utf-8-sig', newline='') as f:
#     writer = csv.writer(f)
#     for i in list(zip(*result)):
#         writer.writerow(i)

# #result_global 파일 생성
with open('result_global_5.csv', 'w', encoding='utf-8-sig', newline='') as f:
    writer = csv.writer(f)
    for i in list(zip(*result)):
        writer.writerow(i)
